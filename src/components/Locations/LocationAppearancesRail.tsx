...
import { SCENE_STATUS_META, type Location } from "@/lib/types";
import type { LocationAppearance } from "@/lib/locations";
import { cn } from "@/lib/utils";

export function LocationAppearancesRail({
  appearances,
  onOpenScene,
}: {
  ...
  if (appearances.length === 0) {
    return (
      <p className="font-[family-name:var(--font-ui)] text-sm leading-relaxed text-[var(--ink-muted)]">
        No scenes yet. Tag this place on the storyboard — appearances will
        gather here as the manuscript grows.
      </p>
    );
  }
          ...
          >
            <span
              className={cn(
                ...
                a.tagged ? "bg-[var(--accent)]" : "bg-[var(--ink-faint)]",
              )}
              aria-hidden
            />
            {showChapter ? (
...
export function LocationsBackLink() {
  return (
    <Link
      href="/locations"
...
export function locationHref(l: Location) {
  return `/locations/${l.id}`;
}
